...
def parse_bandwidth_metrics(powermetrics_parse):
    ...
    for line in bandwidth_metrics:
        if line["name"] in data_fields:
            bandwidth_metrics_dict[line["name"]] = line["value"] / (1e9)
...
def parse_cpu_metrics(powermetrics_parse):
    ...
    for cluster in cpu_clusters:
        ...
        for cpu in cluster["cpus"]:
            ...
            core.append(cpu["cpu"])
            cpu_metric_dict[name + str(cpu["cpu"]) + "_freq_Mhz"] = int(
                cpu["freq_hz"] / (1e6)
            )
            cpu_metric_dict[name + str(cpu["cpu"]) + "_active"] = int(
    ...
    if "E-Cluster_freq_Mhz" not in cpu_metric_dict:
        ...
        cpu_metric_dict["E-Cluster_freq_Mhz"] = max(
            cpu_metric_dict["E0-Cluster_freq_Mhz"],
            cpu_metric_dict["E1-Cluster_freq_Mhz"],
        )
